...
pub fn init(name: &str, provider: Option<&str>, cwd: &Path) -> anyhow::Result<()> {
    ...
    // the merge inputs (the lock is regenerated by Phase 3). See fo-w9ph9.
    GitVcs
        .set_replay_exclusion(&project_dir, Path::new("rwv.lock"))
        .map_err(|e| anyhow::anyhow!("failed to write .gitattributes: {e}"))?;
...
pub fn init_adopt(source: &str, cwd: &Path) -> anyhow::Result<()> {
    ...
    // that already carry the entry are a no-op. See fo-w9ph9.
