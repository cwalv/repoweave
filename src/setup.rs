...
/// Run `rwv setup agents-md` against an already-resolved workspace context.
///
...
/// re-resolve.
pub fn agents_md(ctx: &WorkspaceContext) -> anyhow::Result<()> {
    let dest = ctx.primary_path().join("AGENTS.md");
...
mod tests {
    ...
    fn agents_md_refuses_hand_edited() {
        ...
        let ctx = WorkspaceContext::resolve(&root, None).unwrap();
        let err = agents_md(&ctx).unwrap_err();
