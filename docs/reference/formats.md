...
Generated by `rwv lock` or `rwv update`. Same repository shape as `rwv.toml` but JSON, and `version` holds resolved revisions. When a tag exists at HEAD, the tag name is recorded; otherwise the revision SHA.
...
`web`'s version is untagged (a revision SHA); `server`'s is tagged (`v2.1.0`, human-readable). `repositories[].version` is a tag name when released, a SHA otherwise — the format itself encodes release state.
