...
Generated by `rwv lock` or `rwv update`. Same repository shape as `rwv.yaml` but JSON, and `version` holds resolved revisions. When a tag exists at HEAD, the tag name is recorded; otherwise the revision SHA.
