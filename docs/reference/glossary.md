...
| **Plugin** | An external `rwv-<verb>` executable on `$PATH`, dispatched when a verb isn't built into rwv itself — the low-friction, self-serve extension point for adding a verb without a PR against core. Must not write rwv-owned files; unlike an integration's enforced file-ownership contract, this is a documented rule, not a dispatch-time guard — `rwv doctor` can only flag a violation after the fact. Contrast with **Integration**. See [plugin-boundary](../explanation/joints/plugin-boundary.md). |
