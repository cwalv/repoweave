...
Under `--json`, the output is a bare JSON array (legacy shape; pre-dates the
`$schema` envelope convention). Each element is a `RepoStatus` record:
`path`, `absolute_path`, `branch`, `tip`, `lock_sha`, `relation`, `mid_op`,
`role`, `url`, `project`.

A follow-up will migrate this to the envelope shape
`{ "$schema": "...", "repos": [...] }`; the committed schema today describes
the bare-array shape that actually ships.
...
```
rwv status --json | jq -r '.[] | "\(.path)\t\(.relation)"'
```

Find every repo that has drifted from its lock:

```
rwv status --json | jq '.[] | select(.relation != "ok" and .relation != "no_lock")'
```

Get absolute paths for all forks (e.g. to run a command across each repo):

```
rwv status --json | jq -r '.[] | select(.role == "fork") | .absolute_path'
```
