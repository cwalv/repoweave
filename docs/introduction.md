...
- **A bounded surface for automation.** `rwv prime` advertises the workspace; `rwv explain <verb>` returns a markdown bundle with the verb's JSON Schema embedded; roles (`owned` / `fork` / `dependency` / `reference`) act as a machine-readable allow-list; workweaves isolate the blast radius. Together they give an agent harness everything it needs to drive the workspace without scraping help text.
...
To report a bug, request an integration, or ask a design question, open a [GitHub issue](https://github.com/cwalv/repoweave/issues).
