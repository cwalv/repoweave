...
4. **`rwv.lock` is load-bearing, not a passive snapshot** — advancement verbs consult it as the target. `rwv sync <source>` aligns the current workspace with `<source>`'s committed lock using a chosen strategy (ff, rebase, merge). Both sides must satisfy `rwv check --locked` before the op runs; `rwv abort` rolls back using savepoint refs. Workspace type (primary vs workweave) is not load-bearing — the same verb handles either direction, determined by CWD. The project repo (snapshot-shaped) and manifest repos (divergent-development-shaped) are treated differently: see [Sync](./sync.md) for the project-vs-manifest distinction, the Phase 1 ancestor precondition, and the symmetry/asymmetry of the operation.

   **Per-workspace lock ownership.** Each workspace — primary weave or workweave — owns the `rwv.lock` file that lives in its own copy of the project repo. The workweave's `projects/<name>/` is a worktree of primary's project repo on a per-workweave ephemeral branch, so it has its own working-tree copy of `rwv.lock` independent of primary's. `rwv lock`, `rwv sync`, `rwv check --locked`, and `rwv status` all resolve the lock file relative to **CWD's workspace directory**, not primary's. A `rwv lock` issued from a workweave updates the workweave's lock and leaves primary's untouched; the workweave decides when to propagate by committing the lock and running `rwv sync` from primary (or pushing the workweave's project branch and merging upstream). Tools never write across the boundary implicitly — that is the point of the workweave being a workspace.

5. **Shared refs, per-worktree index and working tree** — git worktrees share branch refs (under `.git/refs/heads/`) but maintain separate index files and working-tree copies. When one worktree commits and advances a branch ref, any sibling worktree whose `HEAD` symbolic ref resolves through the same branch will silently see its effective HEAD change — but its index and on-disk files won't be updated. This produces two related drift classes: *index drift* (the index no longer matches HEAD, so `git status` shows phantom staged changes) and *working-tree drift* (on-disk files still reflect a prior commit, so workers reading those files get stale content). Both are ambient: nothing is done wrong; the shared-refs model produces them silently. `rwv doctor` detects both classes and distinguishes safe-to-fix instances (content matches a reachable committed blob) from live-staged or live-edit instances (content that must not be auto-overwritten). `rwv doctor --fix` remediates the safe class for both index and working tree, and `rwv sync` applies the same post-condition fix after advancing each repo. The shared safety invariant: `--fix` and sync's post-refresh *only* replace content that exactly matches an already-committed blob in the object DAG — no user work is ever silently discarded.
