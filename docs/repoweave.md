...
Generated by `rwv lock`, same format but with resolved SHAs instead of branch names. Optionally records which weave it was generated from:
...
| `rwv lock` | Snapshot repo versions from the current context into the project's `rwv.lock`. |
| `rwv lock-all` | Snapshot repo versions for all projects. |
...
`rwv lock-all` updates lock files for every project on disk, not just the active one. If you've been working in the web-app context and made commits to `server` and `protocol`, `rwv lock-all` also updates the `rwv.lock` in mobile-app's project directory (since both projects reference those repos).

This gives you the monorepo property of "one action captures all state" — distributed across project repos. Each project repo can then be committed independently, and each remains independently bootstrappable via `rwv fetch`.

### `rwv lock`

Lock operates from the current context: if you're in a weave, it reads HEAD SHAs from the worktrees. If you're in the primary, it reads from the regular clones. The lock file is written to the project directory.

The lock file optionally records which weave it was generated from (see [Lock files](#lock-files)). This is metadata for provenance — it doesn't change any behavior.
