...
Conventions for rwv source that aren't enforced by clippy or rustfmt — read this before reviewing or writing PRs.

## No silent fallback on inconsistent state

Where state on disk or in config could be (a) absent/legitimate, (b) present/valid, or (c) present/invalid, the read path must distinguish (b) from (c) and surface (c) loudly. Silent coalescing of (c) into (a)'s default or (b)'s happy path is a bug, not a defensible default.
...
When the silent fallback is genuinely the user's intent (an optional knob), document the default in the field's doc comment and in `docs/reference/formats.md`.
...
Surfaced during the 2026-05-27 review session (`fo-vsldv` epic). Five concrete instances had landed silently and were caught only via cross-cutting audit (`fo-vsldv.1`). The pattern was named and these guidelines added so the same shape isn't re-introduced.
