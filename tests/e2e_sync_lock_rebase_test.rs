//! E2E integration tests for fo-4c8b1: `rwv sync --strategy=rebase` with
//! concurrent lock-bumping workweaves.
//!
...
//!    histories.
//!
//! 3. **Missing `.gitattributes` hard-bails** (`sync_rebase_without_gitattributes_bails_cleanly`):
//!    When the project repo's committed `.gitattributes` does NOT contain
//!    `rwv.lock merge=ours`, `rwv sync --strategy=rebase` must exit non-zero
//!    before starting any git operations. Post-bail assertions: no
//!    `.git/rebase-merge/` directory (git was never invoked), `git status`
//!    clean.
//!
...
//!    grows by exactly 1 (the lock commit).
//!
//! ## Fixture shape
//!
