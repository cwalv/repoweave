...
mod uv_workspace {
    ...
    fn generates_pyproject_toml_with_uv_workspace() {
        ...
        let content = std::fs::read_to_string(root.join("pyproject.toml")).unwrap();
        // Post fo-cnpjy.9: ownership is proven by the per-key `# managed by rwv`
    ...
    // -----------------------------------------------------------------------
    // §6 uv-workspace — scenarios (GREEN: fo-cnpjy.9)
    // -----------------------------------------------------------------------
    //
...
mod cargo_workspace {
    ...
    fn nested_workspace_with_opt_out_drops_repo_from_members() {
        ...
        // Post-port (fo-cnpjy.7): the `# excluded:` comment emitted by the
...
mod s8_cross_port_default_only {
    ...
    // -----------------------------------------------------------------------
    // npm — `name` and `private` (fo-ro3hj.2)
    //
  ...
  "workspaces": ["github/acme/api"]
        ...
        let manifest = make_manifest(vec![("github/acme/api", Role::Owned)]);
        let project = ProjectName::new("different-project-name");
        let config = IntegrationConfig::default();
        let cache = HashMap::new();
        let ctx = make_ctx(root, &project, &manifest, &config, &cache);
    ...
    fn s8_npm_default_only_seeds_on_greenfield() {
        ...
        let manifest = make_manifest(vec![("github/acme/api", Role::Owned)]);
        let project = ProjectName::new("my-workspace");
        let config = IntegrationConfig::default();
        let cache = HashMap::new();
        let ctx = make_ctx(root, &project, &manifest, &config, &cache);
    ...
    // -----------------------------------------------------------------------
    // cargo — `[workspace].resolver` (fo-ro3hj.4)
    //
    // Port-specific equivalents:
    ...
    // -----------------------------------------------------------------------
    // vscode — `git.autoRepositoryDetection` (fo-ro3hj.7)
    //
    // Port-specific equivalents:
